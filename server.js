...
Подбери ${count} материал(а/ов) и дай совет.`;
    ...
    if (!advice) {
      advice = `Для стиля «${styleLabel}» отлично подойдёт ${materials.map((m) => m.name).join(' и ')}. ` +
        `Основной материал нанесите на основную поверхность (стену или фасад), остальное используйте как акцент — так это будет выглядеть дорого и цельно.`;
    }
...
- Если к сообщению приложено ФОТО — сначала по фото коротко определи, это интерьер (комната) или фасад дома, и скажи это в ответе («Вижу гостиную…» или «Вижу фасад вашего дома…»), затем подбирай материалы.
